...
def build_soe_v0_2_molecule(
    ...
    *,
    ...
    summary = {
        ...
        "metrics_present_via_alias": int(present_via_alias),
        "alias_conflicts_detected": int(alias_conflicts),
        "unmapped_related_detected": int(unmapped_related_detected),
    }
    ...
    for mk in metrics_sorted:
        ms = metric_status.get(mk) or {}
        produced_at = ms.get("produced_at")
        created_at = ms.get("created_at")
        ts = _timestamp_used(produced_at, created_at)
        per_metric_rec[mk] = ts
        tv = ts.get("timestamp_value")
        if tv:
            dt = parse_iso(str(tv))
            if dt is not None:
                ...
                if oldest is None or dt < oldest:
                    oldest = dt
                basis_set.add(ts.get("timestamp_used"))
