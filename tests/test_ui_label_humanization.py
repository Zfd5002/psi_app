...
def test_humanize_key_applies_acronyms_and_overrides() -> None:
    ...
    assert humanize_key("advance_to_in_vivo") == "Development Progression"
    assert humanize_key("ready_for_scaleup_screen") == "Scale-Up Readiness Screen"
    assert humanize_key("G1_material_readiness") == "Gate 1: Material Readiness"


def test_humanize_decision_key_uses_scientist_facing_label() -> None:
    assert humanize_decision_key("advance_to_in_vivo") == "Development Progression"
